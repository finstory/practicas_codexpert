...
export const ejerciciosHardocre = () => {
  ...
  class UserModel extends Key {
    ...
    password: string;
    lenguage: string = "TS";
    #isAdmin: boolean; //! no puedes modificar el acceso privado.
  ...
  class AdminModel extends UserModel {
    ...
    constructor(username: string, password: string) {
      ...
      this.role = this.getAdmin() ? "admin" : "user";
    }

    //this.getAdmin? this.role = "admin" : "user"
  }

  const admin = new AdminModel("rama", "rama");
  console.log(admin);
  ...
  interface IMedia {
    extension: "sdsad" | Exclude<IPhoto["extension"] | IVideo["extension"], "avi">;
  }

  // const test: IMedia = {
  //   extension: "", // This will give an error
  // };

  const media: IMedia = {} as any;
